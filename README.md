...
- **[DONE] 动态阈值 (Adaptive VAD)**：基于背景环境音实时计算灵敏度，并支持跨录音片段的全局底噪持久化记忆。
- **[DONE] 语义意图匹配**：基于向量相似度的指令拦截系统。
- **[DONE] 异步响应流**：全面提升并发性能。引入 `LLM_QUEUE` 任务队列与长连接池，杜绝了高并发打断时的资源泄露。
- **[DONE] 雷达感知自愈与重试**：为主动播报加入指数退避重试，并防御性隔离了 HA 脏数据 (`unknown`) 造成的崩溃。
- **[DONE] STT 流水线极速响应**：修复了 WebSocket 断流逻辑，将语音解析延迟降至毫秒级 (< 1s)。
